...
var buildOptions = function(options, parentList, appendVisibleOptions) {
  ...
  for (var i = 0, len = options.length; i < len; i++) {
    ...
    for (var j = 0; j < TOKENS.length; j++) {
        var tokenGroup = TOKENS[j];
        for (var k = 0; k < tokenGroup.length; k++) {
           if (options[i].optionHTML === tokenGroup[k].tokenHTML){
              found = true;
           }
        }
  }
...
// kyle added
var buildHiddenInput = function(tokens) {
  var html = '';
  var data = '';
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    for (var j = 0; j < tokenGroup.length; j++) {
       data += tokenGroup[j].tokenHTML + ',';
    }
  }

  data = data.substr(0, data.length-1);

  html +='<input type="hidden" name="tags" value="' + data + '">'
  return html;
};
...
var updateTokens = function() {
  ...
  tokenHTML += buildHiddenInput(TOKENS);
